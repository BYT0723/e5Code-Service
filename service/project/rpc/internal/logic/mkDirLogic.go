...
func (l *MkDirLogic) MkDir(in *pb.MkDirReq) (*pb.MkDirRsp, error) {
	// 判断Project是否存在
	p := &model.Project{}
	if err := l.svcCtx.DB.Where("id = ?", in.Id).First(p).Error; err != nil {
		...
		logx.Error("Fail to Find Project on mkDir: ", err.Error())
	...
	if err != nil {
		logx.Error("Fail to Open Repository on mkDir:", err.Error())
		return nil, status.Error(codesx.GitError, err.Error())
	}
	if err := gitx.MkDir(rep, in.Path); err != nil {
		logx.Error("Fail to Mkdir:", err.Error())
		return nil, status.Error(codesx.GitError, err.Error())
	}
	return &pb.MkDirRsp{}, nil
}
